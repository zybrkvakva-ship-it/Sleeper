...
object SolanaPda {
    ...
    fun findProgramAddress(seeds: List<ByteArray>, programId: ByteArray): Pair<ByteArray, Int>? {
        ...
        for (bump in 255 downTo 0) {
            ...
            if (pda != null) {
                DevLog.d(TAG, "findProgramAddress: found at bump=$bump pdaHex=${pda.hex()} pdaBase58=${com.seekerminer.app.utils.Base58.encode(pda)}")
    ...
    fun getReverseLookupPda(nameAccountPubkeyBase58: String, tldHouseBytes: ByteArray): ByteArray? {
        ...
        DevLog.d(TAG, "getReverseLookupPda: result=${if (result != null) "ok base58=${com.seekerminer.app.utils.Base58.encode(result)}" else "null"}")
        return result
    }
}
